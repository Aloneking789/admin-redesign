  ...
  const mockProviders: Provider[] = [
    {
      ...
      completedJobs: 156,
      verificationDocuments: ['business_license.pdf', 'insurance.pdf']
    },
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {mockProviders.map((provider) => (
              <tr key={provider.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusColors[provider.status]}`}>
                    {provider.status}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <div className="flex flex-wrap gap-1">
                    {provider.services.map((service) => (
                      <span key={service} className="px-2 py-1 text-xs bg-gray-100 rounded-full">
                        {service}
                      </span>
                    ))}
                  </div>
